...
function UpdateStudentdetail() {
  ...
  const handleChange1 = (e, index) => {
    ...
    if (name === 'amount' || name === 'installment_date' || name === 'p_status') {
      setVal((prevVal) => {
    ...
    if (name === 'p_status') {
      setRemainingFees(remaining_fees - parseInt(val.installment_details[index].amount))
    }
  };
  ...
  const handleSubmit = () => {
    ...
    const setUnpaidfee = val.installment_details.filter((item) => item.p_status === '0');
    const total_unpaidFees = val.installment_details.filter((item) => item.p_status === '0').reduce((total, item) => total + parseInt(item.amount), 0);
    ...
    if (remaining_fees === total_unpaidFees) {

      axios.put(`http://localhost:5000/course/updatestudentDetail/${param.id}`, {
        surname: val.surname,
        ...
        .then(function (response) {
          ...
          if (response.data.status === 'Update Student_Data Successfully') {
            handleClickOpen();

          }
          else {
            setErmessage("Your Data is Update Successfully")
            setOpen1(true);
            setTimeout(() => {
              setOpen1(false)
            }, 5000);
      ...
      setErmessage("Your Amount and Remainning Fees total are different")
      setOpen1(true);
      setTimeout(() => {
        setOpen1(false)
      }, 5000);
        ...
        <Box sx={{ width: '100%', typography: 'body1' }}>
          <TabContext value={value}>
            ...
            <form>
                    ...
                    <Box sx={{ display: 'flex' }}>
                      <Box xs={{ mt: 5 }} >
                        <Grid container spacing={2}>
                          ...
                          <Grid item xs={12} md={4} sx={{ marginTop: '0px', marginBottom: '5px!important', paddingTop: '0!important', textAlign: 'left' }}>
                            <TextField
                              InputLabelProps={{ shrink: true }}
                          ...
                          <Grid item xs={12} md={12} sx={{ marginBottom: '5px!important', paddingTop: '0!important' }}>
                            ...
                            <Grid item xs={12} md={12}
                              ...
                              {val.installment_details && val.installment_details.map((c, index) => {
                                return (
                                  <Grid container spacing={2} key={index} >
                                    ...
                                    <Grid item xs={10} md={2} sx={{ marginBottom: '20px!important', paddingTop: '17px!important', paddingLeft: '40px!important' }}>

                                      <FormControl sx={{ minWidth: 150 }}>
                                        <InputLabel id="demo-controlled-open-select-label">Payment Status</InputLabel>
                                        <Select
                                          labelId="demo-controlled-open-select-label"
                                          id="demo-controlled-open-select"
                                          disabled={c.p_status === '1'}
                                    ...
                                    <Grid item xs={10} md={2} sx={{ marginBottom: '20px!important', paddingTop: '3px!important', textAlign: 'center' }}>

                                      <Button variant="contained" type="button" sx={{ marginTop: '14px', mb: 2, padding: '7px 20px', backgroundColor: 'red', color: 'white' }} onClick={handleDelete} > <DeleteForeverIcon /> </Button>
                                    </Grid>
                                  </Grid>
              ...
              <TabPanel value="3"> <Grid item xs={12} md={12} component={Paper}
                elevation={10}
                square margin='auto' sx={{ borderRadius: "10px", padding: "20px 30px" }}>
                <Box sx={{ display: 'flex', flexDirection: 'column', justifyContent: 'center', alignItems: "center" }}>
                  <Typography variant="h4" sx={{ mt: 1, mb: 4, fontWeight: 'bold' }}> Fees Details</Typography>
                  <Box sx={{ display: 'flex' }}>
                    <Box xs={{ mt: 5 }} >
                      <Grid container spacing={2}>
          ...
          <Box sx={{ padding: '0 40px 20px 40px!important' }}>
            ...
            <DialogContent>
              <DialogContentText id="alert-dialog-slide-description">
                Your Admission Update Successfully Saved!
              </DialogContentText>
            </DialogContent>
            <DialogActions >
